...
| `status` | `TEXT` | `NOT NULL`, default `success` | 调用状态（`success \| error \| denied`） |
...
索引：
- 普通索引 `tool_execution_record_floor_created_idx(floor_id, created_at)`
- 普通索引 `tool_execution_record_run_idx(run_id)`
- 普通索引 `tool_execution_record_page_created_idx(page_id, created_at)`
