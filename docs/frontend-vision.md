# TavernHeadless Frontend Vision
...
## 9. 视觉基调参考 (Visual Tone)

我们要打造的不是一个 CRUD 后台，而是一个**精密的光学仪器**。

### 9.1 核心美学：Atmospheric Utilitarianism (氛围实用主义)
...
**基调 CSS 变量示意 (Tailwind Theme Extend):**
...
*/
:root {
  /* 空间构建：使用不透明度和层叠，而非绝对灰度 */
  --app-bg: #09090b;             /* Zinc-950: 深邃，但不死黑 */
  --panel-base: #121215;         /* 略微抬升的基底 */
  --panel-float: #18181b;        /* 悬浮层 */
  ...
  /* 信号色：低饱和，高亮度，像旧示波器 */
  --signal-success: #4ade80;     /* Emerald-400 */
  --signal-warn: #fbbf24;        /* Amber-400 */
  --signal-error: #f87171;       /* Red-400 */
  --signal-accent: #2dd4bf;      /* Teal-400: 理性、冷静的连接色 */
  
  /* 质感 */
  --glass-blur: 12px;
  --font-mono: 'JetBrains Mono', monospace; /* 数据的唯一载体 */
}
...
这个基调避免了“企业级后台”的沉闷，也避免了“赛博朋克”的喧闹，保持了一种“专业工具”的静谧感。
...
可视化关系建议：`Session -> Character (1)` 与 `Session -> Worldbooks (N)`，而不是三者并列入口。

### 10.2 布局映射（避免混乱）

- 左栏第一优先展示会话列表，角色卡与世界书归入 `Library` 分组。
- 中栏只承载当前会话的创作与调试，不在主画布做跨实体编辑。
- 右栏提供 `Bindings` 面板，展示“当前会话绑定了谁、命中了谁、优先级如何”。

### 10.3 CRUD 语义分层（关键）

- **Create**：在会话流中是“新建会话并绑定角色/世界书”；在 Library 中是“创建可复用资产”。
- **Read**：会话内读取的是“绑定后的生效视图”；资产详情读取的是“原始定义”。
- **Update**：会话内改绑定关系（Attach/Detach/Priority），资产编辑在 Side Sheet 完成，保存后回流会话。
- **Delete**：默认删除“绑定关系”而非删除资产本体；删除资产需二次确认并显示影响会话数。

### 10.4 关键交互细则

- 在会话标题区固定显示 `Active Character` 与 `Active Worldbooks` 标签，保持用户全程上下文感知。
- 绑定操作使用动词化按钮：`Attach`, `Detach`, `Replace`, `Set Priority`，避免模糊的“编辑”。
- 所有危险操作先给 Impact Preview（例如“该世界书被 12 个会话引用”），再确认。
...
关系可视化建议：`Account -> Session -> User(1) + Character(1) + Worldbooks(N)`。
