...
export function serializeSessionToThChat(
  ...
  if (includeMemories) {
    ...
    const exportItems: ThChatMemoryItem[] = memItems.map((m) => ({
      ...
      status: m.status,
      lifecycle_status: m.lifecycleStatus ?? (m.status === "deprecated" ? "deprecated" : "active"),
      source_job_id: m.sourceJobId,
      token_count_estimate: m.tokenCountEstimate,
      last_used_at: m.lastUsedAt,
      coverage_start_floor_no: m.coverageStartFloorNo,
      coverage_end_floor_no: m.coverageEndFloorNo,
      derived_from_count: m.derivedFromCount,
      created_at: m.createdAt,
